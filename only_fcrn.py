...
# 解码器的损失函数：
loss_decoder = tf.reduce_mean(((real_map-label)**2))
# 噪声发生器的损失：
...
#设置优化器
optimizer = tf.train.AdamOptimizer(lr).minimize(loss_decoder, global_step=global_step)
...
#         喂入网络的数据
        feeds = {image: feed_image, label: feed_label}
#         训练网络，获取相关信息
        _,input_image,output_real_map, input_label,decoder_loss_output,learningrate = sess.run([optimizer,image,real_map,
                                 label,loss_decoder,lr], feeds)
    
#           打印当前的损失
        print('epoch:{} decoder_loss:{}  lr:{}'.format(i+start_epoch,decoder_loss_output, learningrate))
        end = time.time()
